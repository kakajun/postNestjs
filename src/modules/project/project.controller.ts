...
export class ProjectController {
  ...
  async hall(
    ...
    if (longitude != null && latitude != null) {
      ...
      if (projectIds.length) {
        const annexes = await this.annexRepo.createQueryBuilder('a').where('a.project_id IN (:...ids)', { ids: projectIds }).select(['a.id', 'a.projectId', 'a.url', 'a.expireTime']).getMany()
  ...
  async detail(@Param('id') id: string) {
    ...
    if (!proj) return null
    const annexes = await this.annexRepo.find({ where: { projectId: id }, select: ['id', 'url', 'expireTime'] })
    const now = Date.now()
    annexes.forEach(a => { if (a.expireTime && new Date(a.expireTime).getTime() < now) a.url = '' })
  ...
  async add(@UploadedFiles() files: Express.Multer.File[], @Body() body: any) {
    ...
    if (files && files.length) {
      ...
      for (const file of files) {
        ...
        try { thumbnail = await sharp(file.buffer).resize({ width: 100 }).jpeg().toBuffer() } catch { thumbnail = file.buffer }
        const url = await minio.presignedGetObject(bucket, obj, Number(process.env.MINIO_EXPIRE || 3600))
        await this.annexRepo.save({ id: Date.now().toString(), projectId: id, name, path: obj, thumbnail, url })
      }
    }
    return true
  }
  ...
  async update(@UploadedFiles() files: Express.Multer.File[], @Body() body: any) {
    ...
    await this.projectRepo.update({ id }, { name: body?.projectName || project.name, technology: body?.technology || project.technology, request: body?.request || project.request, auditStatus: 0, auditRemark: '', updateTime: new Date() })
    if (files && files.length) {
      const minio = createMinio()
      const bucket = process.env.MINIO_BUCKET || 'wb-bucket'
      for (const file of files) {
  ...
  @Get('my/audit/list')
  async myAuditList(@Query('pageNo') pageNo = 1, @Query('pageSize') pageSize = 10, @Query() query: any, @Body() _body: any, @Param() _param: any, @UploadedFiles() _f?: any) {
    ...
    const page = Number(pageNo), size = Number(pageSize)
